...
type BotTradeHistoryTelegramProps = {
...
const BotTradeHistoryTelegram: FC<BotTradeHistoryTelegramProps> = ({
  isBotTelegram,
  setIsBotTelegram,
}) => {
    ...
    (page: number, search?: string) => {
      ...
      noLoadingAxios
        .get(`${APIs.orderBotList}`, {
          params: {
            page,
            itele : 1, // Adding filter condition,
            page_size: selectedPageSizeOption.value,
            queryString: search || "",
            //master_gt: 10 // Adding filter condition
          },
        })
        .then((res) => {
  ...
  const deleteHistoryByListIds = () => {
   ...
   useEffect(() => {
    const timer = setTimeout( () => {
      getAutoBotOrderHistory(page + 1, searchInput);
    }, 500);
...
export default BotTradeHistoryTelegram;
