...
import {
  ...
  AutoBotHistoryTelegramListResult,
...
type BotTradeHistoryMethodProps = {
  isBotTelegram: boolean;
  setIsBotTelegram: React.Dispatch<React.SetStateAction<boolean>>;
};
...
const BotTradeHistoryMethod: FC<BotTradeHistoryMethodProps> = ({
  isBotTelegram,
  setIsBotTelegram,
}) => {
  ...
  const [autoBotHistoryTelegram, setAutoBotHistoryTelegram] =
    useState<AutoBotHistoryTelegramList>({
      count: 0,
      next: null,
      previous: null,
      results: [],
  ...
  const data = React.useMemo<AutoBotHistoryTelegramListResult[]>(
    () => autoBotHistoryTelegram.results,
    [autoBotHistoryTelegram]
  );
  ...
  useEffect(() => {
    const delay = 500; // Adjust the debounce delay as needed (in milliseconds)
  ...
  useEffect(() => {
    ...
    noLoadingAxios
      ...
      .then((res) => {
        ...
        setAutoBotHistoryTelegram(data);
        if (data.count != null && selectedPageSizeOption.value != null) {
          setTotalPages(Math.ceil(data.count / +selectedPageSizeOption.value));
        }
      })
      .catch(() => {
      ...
      noLoadingAxios
        .get(`${APIs.orderBotList}`, {
          params: {
            page,
            page_size: selectedPageSizeOption.value,
            itele : 0  ,// Adding filter condition
            queryString: debouncedValue
          },
        })
        .then((res) => {
  ...
  }, [dispatch, getAutoBotOrderHistory, page]);
  ...
  const columns = React.useMemo<Column<AutoBotHistoryTelegramListResult>[]>(
    () => [
      {
        id: 'checkbox',
        accessor: 'checkbox',
      ...
      },
      {
        ...
        Cell: (props) => {
          const original = props.cell.row.original;
          // return <p className="text-xs text-ink-100">{original.config_name}</p>;
          return <p className="text-sm text-ink-100">{original.master_name}</p>;
        },
      },
      {
        Header: () => {
      ...
      // {
      //   Header: () => <div className="text-end">Lãi con/LN</div>,
      //   accessor: 'young_profit',
      //   Cell: (props) => {
      //     const original = props.cell.row.original;
      //     return (
      ...
      //         {/* <span
      //           className={`font-bold  ${
      //             original.young_profit >= 0 ? 'text-green-100' : 'text-red-100'
      //           }`}
      //         >
      //           {original.young_profit >= 0 ? '$' : '-$'}
      //           {Math.abs(original.young_profit).toLocaleString(undefined, {
      //             minimumFractionDigits: 0,
      //             maximumFractionDigits: 3,
      //           })}
      //         </span> */}
      //         <span className="text-green-100">$0</span>/
      //         <span className="text-red-100">-$5,000</span>
      //       </div>
      //     );
      //   },
      // },
            ...
            <div className="text-end">
              {isWinning && (
                <span className="text-green-100 rounded-3xl text-sm font-bold">
                  +$
                  {(original.o_amount * BASE_RATE).toLocaleString('en-US', options)}
                </span>
              )}
              {isLosing && (
  ...
  useEffect(() => {
    if (!isInitialized && autoBotHistoryTelegram.results.length > 0) {
      setPartnerBotId(data[0].o_PartnerAcc_owner);
      setIsInitialized(true);
    }
  }, [autoBotHistoryTelegram.results.length, data, isInitialized]);
  ...
  return (
    <>
      <div className="h-fit mb-6 bg-background-80 rounded-3xl">
        <div className="flex gap-y-6 px-6 pt-2 border-b border-ink-10">
          <div
            className={`relative py-4 px-6 cursor-pointer ${
              isBotTelegram ? 'border-bottom-primary' : ''
            }`}
            onClick={() => setIsBotTelegram(true)}
          >
            <p
              className={`${
                isBotTelegram
          ...
          <div
            ...
            onClick={() => setIsBotTelegram(false)}
          >
            <p
              className={`${
                !isBotTelegram
        ...
        {autoBotHistoryTelegram.results.length === 0 && (
...
export default BotTradeHistoryMethod;
